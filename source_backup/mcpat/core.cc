 ...
 exist(exist_)
{
	  ...
	  data							   = XML->sys.machine_bits;//64 is the data width
	  interface_ip.is_cache			   = true;
...
void BranchPredictor::computeEnergy(bool is_tdp)
{
    ...
    else
    {
    	...
    	w_access = 0;//XML->sys.core[ithCore].branch_mispredictions + 0.5*XML->sys.core[ithCore].branch_instructions;
    	globalBPT->stats_t.readAc.access  = r_access;
    	globalBPT->stats_t.writeAc.access = w_access;
...
void BranchPredictor::displayEnergy(uint32_t indent,int plevel,bool is_tdp)
{
	...
	if (is_tdp)
	{
		cerr << indent_str<< "Global Predictor:" << endl;
		cerr << indent_str_next << "Area = " << globalBPT->area.get_area()*1e-6<< " mm^2" << endl;
		cerr << indent_str_next << "Peak Dynamic = " << globalBPT->power.readOp.dynamic*clockRate << " W" << endl;
		...
		cerr << indent_str_next << "Gate Leakage = " << globalBPT->power.readOp.gate_leakage << " W" << endl;
		cerr << indent_str_next << "Runtime Dynamic = " << globalBPT->rt_power.readOp.dynamic/executionTime << " W" << endl;
		cerr <<endl;
		cerr << indent_str << "Local Predictor:" << endl;
		cerr << indent_str_next << "Area = " << L1_localBPT->area.get_area() *1e-6 << " mm^2" << endl;
		cerr << indent_str_next << "Peak Dynamic = " << L1_localBPT->power.readOp.dynamic*clockRate  << " W" << endl;
		cerr << indent_str_next << "Subthreshold Leakage = "
			<< (long_channel? L1_localBPT->power.readOp.longer_channel_leakage:L1_localBPT->power.readOp.leakage)  << " W" << endl;
		cerr << indent_str_next << "Gate Leakage = " << L1_localBPT->power.readOp.gate_leakage  << " W" << endl;
		cerr << indent_str_next << "Runtime Dynamic = " << L1_localBPT->rt_power.readOp.dynamic/executionTime << " W" << endl;
		cerr <<endl;

		cerr << indent_str_next << "Area = " << L2_localBPT->area.get_area() *1e-6 << " mm^2" << endl;
		cerr << indent_str_next << "Peak Dynamic = " << L2_localBPT->power.readOp.dynamic*clockRate  << " W" << endl;
		cerr << indent_str_next << "Subthreshold Leakage = "
			<< (long_channel? L2_localBPT->power.readOp.longer_channel_leakage:L2_localBPT->power.readOp.leakage)  << " W" << endl;
		cerr << indent_str_next << "Gate Leakage = " << L2_localBPT->power.readOp.gate_leakage  << " W" << endl;
		cerr << indent_str_next << "Runtime Dynamic = " << L2_localBPT->rt_power.readOp.dynamic/executionTime << " W" << endl;
		cerr <<endl;

		cerr << indent_str << "Chooser:" << endl;
		cerr << indent_str_next << "Area = " << chooser->area.get_area()  *1e-6 << " mm^2" << endl;
		cerr << indent_str_next << "Peak Dynamic = " << chooser->power.readOp.dynamic*clockRate  << " W" << endl;
		cerr << indent_str_next << "Subthreshold Leakage = "
			<< (long_channel? chooser->power.readOp.longer_channel_leakage:chooser->power.readOp.leakage)  << " W" << endl;
		cerr << indent_str_next << "Gate Leakage = " << chooser->power.readOp.gate_leakage  << " W" << endl;
		cerr << indent_str_next << "Runtime Dynamic = " << chooser->rt_power.readOp.dynamic/executionTime << " W" << endl;
		cerr <<endl;
		cerr << indent_str << "RAS:" << endl;
		cerr << indent_str_next << "Area = " << RAS->area.get_area() *1e-6 << " mm^2" << endl;
		cerr << indent_str_next << "Peak Dynamic = " << RAS->power.readOp.dynamic*clockRate  << " W" << endl;
		cerr << indent_str_next << "Subthreshold Leakage = "
			<< (long_channel? RAS->power.readOp.longer_channel_leakage:RAS->power.readOp.leakage)  << " W" << endl;
		cerr << indent_str_next << "Gate Leakage = " << RAS->power.readOp.gate_leakage  << " W" << endl;
		cerr << indent_str_next << "Runtime Dynamic = " << RAS->rt_power.readOp.dynamic/executionTime << " W" << endl;
		cerr <<endl;
	}
	else
	{
//		cerr << indent_str_next << "Global Predictor    Peak Dynamic = " << globalBPT->rt_power.readOp.dynamic*clockRate << " W" << endl;
//		cerr << indent_str_next << "Global Predictor    Subthreshold Leakage = " << globalBPT->rt_power.readOp.leakage <<" W" << endl;
//		cerr << indent_str_next << "Global Predictor    Gate Leakage = " << globalBPT->rt_power.readOp.gate_leakage << " W" << endl;
//		cerr << indent_str_next << "Local Predictor   Peak Dynamic = " << L1_localBPT->rt_power.readOp.dynamic*clockRate  << " W" << endl;
//		cerr << indent_str_next << "Local Predictor   Subthreshold Leakage = " << L1_localBPT->rt_power.readOp.leakage  << " W" << endl;
//		cerr << indent_str_next << "Local Predictor   Gate Leakage = " << L1_localBPT->rt_power.readOp.gate_leakage  << " W" << endl;
//		cerr << indent_str_next << "Chooser   Peak Dynamic = " << chooser->rt_power.readOp.dynamic*clockRate  << " W" << endl;
//		cerr << indent_str_next << "Chooser   Subthreshold Leakage = " << chooser->rt_power.readOp.leakage  << " W" << endl;
//		cerr << indent_str_next << "Chooser   Gate Leakage = " << chooser->rt_power.readOp.gate_leakage  << " W" << endl;
//		cerr << indent_str_next << "RAS   Peak Dynamic = " << RAS->rt_power.readOp.dynamic*clockRate  << " W" << endl;
//		cerr << indent_str_next << "RAS   Subthreshold Leakage = " << RAS->rt_power.readOp.leakage  << " W" << endl;
//		cerr << indent_str_next << "RAS   Gate Leakage = " << RAS->rt_power.readOp.gate_leakage  << " W" << endl;
	}
...
void InstFetchU::displayEnergy(uint32_t indent,int plevel,bool is_tdp)
{
	...
	if (is_tdp)
	{
		...
		cerr << indent_str<< "Instruction Cache:" << endl;
		cerr << indent_str_next << "Area = " << icache.area.get_area()*1e-6<< " mm^2" << endl;
		cerr << indent_str_next << "Peak Dynamic = " << icache.power.readOp.dynamic*clockRate << " W" << endl;
		cerr << indent_str_next << "Subthreshold Leakage = "
			<< (long_channel? icache.power.readOp.longer_channel_leakage:icache.power.readOp.leakage) <<" W" << endl;
		cerr << indent_str_next << "Gate Leakage = " << icache.power.readOp.gate_leakage << " W" << endl;
		cerr << indent_str_next << "Runtime Dynamic = " << icache.rt_power.readOp.dynamic/executionTime << " W" << endl;
		cerr <<endl;
		if (coredynp.predictionW>0)
		{
			cerr << indent_str<< "Branch Target Buffer:" << endl;
			cerr << indent_str_next << "Area = " << BTB->area.get_area() *1e-6 << " mm^2" << endl;
			cerr << indent_str_next << "Peak Dynamic = " << BTB->power.readOp.dynamic*clockRate  << " W" << endl;
			cerr << indent_str_next << "Subthreshold Leakage = "
				<< (long_channel? BTB->power.readOp.longer_channel_leakage:BTB->power.readOp.leakage)  << " W" << endl;
			cerr << indent_str_next << "Gate Leakage = " << BTB->power.readOp.gate_leakage  << " W" << endl;
			cerr << indent_str_next << "Runtime Dynamic = " << BTB->rt_power.readOp.dynamic/executionTime << " W" << endl;
			cerr <<endl;
			if (BPT->exist)
			{
				cerr << indent_str<< "Branch Predictor:" << endl;
				cerr << indent_str_next << "Area = " << BPT->area.get_area()  *1e-6<< " mm^2" << endl;
				cerr << indent_str_next << "Peak Dynamic = " << BPT->power.readOp.dynamic*clockRate  << " W" << endl;
				cerr << indent_str_next << "Subthreshold Leakage = "
					<< (long_channel? BPT->power.readOp.longer_channel_leakage:BPT->power.readOp.leakage)  << " W" << endl;
				cerr << indent_str_next << "Gate Leakage = " << BPT->power.readOp.gate_leakage  << " W" << endl;
				cerr << indent_str_next << "Runtime Dynamic = " << BPT->rt_power.readOp.dynamic/executionTime << " W" << endl;
				cerr <<endl;
				if (plevel>3)
				{
		...
		cerr << indent_str<< "Instruction Buffer:" << endl;
		cerr << indent_str_next << "Area = " << IB->area.get_area()*1e-6  << " mm^2" << endl;
		cerr << indent_str_next << "Peak Dynamic = " << IB->power.readOp.dynamic*clockRate  << " W" << endl;
		cerr << indent_str_next << "Subthreshold Leakage = "
		<< (long_channel? IB->power.readOp.longer_channel_leakage:IB->power.readOp.leakage)  << " W" << endl;
		cerr << indent_str_next << "Gate Leakage = " << IB->power.readOp.gate_leakage  << " W" << endl;
		cerr << indent_str_next << "Runtime Dynamic = " << IB->rt_power.readOp.dynamic/executionTime << " W" << endl;
		cerr <<endl;
		cerr << indent_str<< "Instruction Decoder:" << endl;
		cerr << indent_str_next << "Area = " << (ID_inst->area.get_area() +
				ID_operand->area.get_area() +
				ID_misc->area.get_area())*coredynp.decodeW*1e-6  << " mm^2" << endl;
		cerr << indent_str_next << "Peak Dynamic = " << (ID_inst->power.readOp.dynamic +
		...
		cerr << indent_str_next << "Gate Leakage = " << (ID_inst->power.readOp.gate_leakage +
				ID_operand->power.readOp.gate_leakage +
				ID_misc->power.readOp.gate_leakage)  << " W" << endl;
		cerr << indent_str_next << "Runtime Dynamic = " << (ID_inst->rt_power.readOp.dynamic +
	...
	{
//		cerr << indent_str_next << "Instruction Cache    Peak Dynamic = " << icache.rt_power.readOp.dynamic*clockRate << " W" << endl;
//		cerr << indent_str_next << "Instruction Cache    Subthreshold Leakage = " << icache.rt_power.readOp.leakage <<" W" << endl;
//		cerr << indent_str_next << "Instruction Cache    Gate Leakage = " << icache.rt_power.readOp.gate_leakage << " W" << endl;
//		cerr << indent_str_next << "Instruction Buffer   Peak Dynamic = " << IB->rt_power.readOp.dynamic*clockRate  << " W" << endl;
//		cerr << indent_str_next << "Instruction Buffer   Subthreshold Leakage = " << IB->rt_power.readOp.leakage  << " W" << endl;
//		cerr << indent_str_next << "Instruction Buffer   Gate Leakage = " << IB->rt_power.readOp.gate_leakage  << " W" << endl;
//		cerr << indent_str_next << "Branch Target Buffer   Peak Dynamic = " << BTB->rt_power.readOp.dynamic*clockRate  << " W" << endl;
//		cerr << indent_str_next << "Branch Target Buffer   Subthreshold Leakage = " << BTB->rt_power.readOp.leakage  << " W" << endl;
//		cerr << indent_str_next << "Branch Target Buffer   Gate Leakage = " << BTB->rt_power.readOp.gate_leakage  << " W" << endl;
//		cerr << indent_str_next << "Branch Predictor   Peak Dynamic = " << BPT->rt_power.readOp.dynamic*clockRate  << " W" << endl;
//		cerr << indent_str_next << "Branch Predictor   Subthreshold Leakage = " << BPT->rt_power.readOp.leakage  << " W" << endl;
//		cerr << indent_str_next << "Branch Predictor   Gate Leakage = " << BPT->rt_power.readOp.gate_leakage  << " W" << endl;
	}
	...
	else
	{
		if (coredynp.core_ty==OOO)
		{
			if (coredynp.scheu_ty==PhysicalRegFile)
			{
				...
				rt_power	      =  rt_power + (iFRAT->power_t + fFRAT->power_t)
...
void RENAMINGU::displayEnergy(uint32_t indent,int plevel,bool is_tdp)
{
	...
	if (is_tdp)
	{
		...
		if (coredynp.core_ty==OOO)
		{
			cerr << indent_str<< "Int Front End RAT:" << endl;
			cerr << indent_str_next << "Area = " << iFRAT->area.get_area()*1e-6<< " mm^2" << endl;
			cerr << indent_str_next << "Peak Dynamic = " << iFRAT->power.readOp.dynamic*clockRate << " W" << endl;
			cerr << indent_str_next << "Subthreshold Leakage = "
				<< (long_channel? iFRAT->power.readOp.longer_channel_leakage:iFRAT->power.readOp.leakage) <<" W" << endl;
			cerr << indent_str_next << "Gate Leakage = " << iFRAT->power.readOp.gate_leakage << " W" << endl;
			cerr << indent_str_next << "Runtime Dynamic = " << iFRAT->rt_power.readOp.dynamic/executionTime << " W" << endl;
			cerr <<endl;
			cerr << indent_str<< "FP Front End RAT:" << endl;
			cerr << indent_str_next << "Area = " << fFRAT->area.get_area()*1e-6  << " mm^2" << endl;
			cerr << indent_str_next << "Peak Dynamic = " << fFRAT->power.readOp.dynamic*clockRate  << " W" << endl;
			cerr << indent_str_next << "Subthreshold Leakage = "
				<< (long_channel? fFRAT->power.readOp.longer_channel_leakage:fFRAT->power.readOp.leakage)  << " W" << endl;
			cerr << indent_str_next << "Gate Leakage = " << fFRAT->power.readOp.gate_leakage  << " W" << endl;
			cerr << indent_str_next << "Runtime Dynamic = " << fFRAT->rt_power.readOp.dynamic/executionTime << " W" << endl;
			cerr <<endl;
			cerr << indent_str<<"Free List:" << endl;
			cerr << indent_str_next << "Area = " << ifreeL->area.get_area()*1e-6  << " mm^2" << endl;
			cerr << indent_str_next << "Peak Dynamic = " << ifreeL->power.readOp.dynamic*clockRate  << " W" << endl;
			cerr << indent_str_next << "Subthreshold Leakage = "
				<< (long_channel? ifreeL->power.readOp.longer_channel_leakage:ifreeL->power.readOp.leakage)  << " W" << endl;
			cerr << indent_str_next << "Gate Leakage = " << ifreeL->power.readOp.gate_leakage  << " W" << endl;
			cerr << indent_str_next << "Runtime Dynamic = " << ifreeL->rt_power.readOp.dynamic/executionTime << " W" << endl;
			cerr <<endl;

			if (coredynp.scheu_ty==PhysicalRegFile)
			{
				cerr << indent_str<< "Int Retire RAT: " << endl;
				cerr << indent_str_next << "Area = " << iRRAT->area.get_area() *1e-6 << " mm^2" << endl;
				cerr << indent_str_next << "Peak Dynamic = " << iRRAT->power.readOp.dynamic*clockRate  << " W" << endl;
				cerr << indent_str_next << "Subthreshold Leakage = "
					<< (long_channel? iRRAT->power.readOp.longer_channel_leakage:iRRAT->power.readOp.leakage)  << " W" << endl;
				cerr << indent_str_next << "Gate Leakage = " << iRRAT->power.readOp.gate_leakage  << " W" << endl;
				cerr << indent_str_next << "Runtime Dynamic = " << iRRAT->rt_power.readOp.dynamic/executionTime << " W" << endl;
				cerr <<endl;
				cerr << indent_str<< "FP Retire RAT:" << endl;
				cerr << indent_str_next << "Area = " << fRRAT->area.get_area()  *1e-6<< " mm^2" << endl;
				cerr << indent_str_next << "Peak Dynamic = " << fRRAT->power.readOp.dynamic*clockRate  << " W" << endl;
				cerr << indent_str_next << "Subthreshold Leakage = "
					<< (long_channel? fRRAT->power.readOp.longer_channel_leakage:fRRAT->power.readOp.leakage)  << " W" << endl;
				cerr << indent_str_next << "Gate Leakage = " << fRRAT->power.readOp.gate_leakage  << " W" << endl;
				cerr << indent_str_next << "Runtime Dynamic = " << fRRAT->rt_power.readOp.dynamic/executionTime << " W" << endl;
				cerr <<endl;
				cerr << indent_str<< "FP Free List:" << endl;
				cerr << indent_str_next << "Area = " << ffreeL->area.get_area()*1e-6  << " mm^2" << endl;
				cerr << indent_str_next << "Peak Dynamic = " << ffreeL->power.readOp.dynamic*clockRate  << " W" << endl;
				cerr << indent_str_next << "Subthreshold Leakage = "
					<< (long_channel? ffreeL->power.readOp.longer_channel_leakage:ffreeL->power.readOp.leakage)  << " W" << endl;
				cerr << indent_str_next << "Gate Leakage = " << ffreeL->power.readOp.gate_leakage  << " W" << endl;
				cerr << indent_str_next << "Runtime Dynamic = " << ffreeL->rt_power.readOp.dynamic/executionTime << " W" << endl;
				cerr <<endl;
			}
		}
		else
		{
			cerr << indent_str<< "Int DCL:" << endl;
			cerr << indent_str_next << "Peak Dynamic = " << idcl->power.readOp.dynamic*clockRate  << " W" << endl;
			cerr << indent_str_next << "Subthreshold Leakage = "
				<< (long_channel? idcl->power.readOp.longer_channel_leakage:idcl->power.readOp.leakage)  << " W" << endl;
			cerr << indent_str_next << "Gate Leakage = " << idcl->power.readOp.gate_leakage  << " W" << endl;
			cerr << indent_str_next << "Runtime Dynamic = " << idcl->rt_power.readOp.dynamic/executionTime << " W" << endl;
			cerr << indent_str<<"FP DCL:" << endl;
			cerr << indent_str_next << "Peak Dynamic = " << fdcl->power.readOp.dynamic*clockRate  << " W" << endl;
			cerr << indent_str_next << "Subthreshold Leakage = "
				<< (long_channel? fdcl->power.readOp.longer_channel_leakage:fdcl->power.readOp.leakage)  << " W" << endl;
			cerr << indent_str_next << "Gate Leakage = " << fdcl->power.readOp.gate_leakage  << " W" << endl;
			cerr << indent_str_next << "Runtime Dynamic = " << fdcl->rt_power.readOp.dynamic/executionTime << " W" << endl;
		}
	}
	else
	{
		if (coredynp.core_ty==OOO)
		{
			cerr << indent_str_next << "Int Front End RAT    Peak Dynamic = " << iFRAT->rt_power.readOp.dynamic*clockRate << " W" << endl;
			cerr << indent_str_next << "Int Front End RAT    Subthreshold Leakage = " << iFRAT->rt_power.readOp.leakage <<" W" << endl;
			cerr << indent_str_next << "Int Front End RAT    Gate Leakage = " << iFRAT->rt_power.readOp.gate_leakage << " W" << endl;
			cerr << indent_str_next << "FP Front End RAT   Peak Dynamic = " << fFRAT->rt_power.readOp.dynamic*clockRate  << " W" << endl;
			cerr << indent_str_next << "FP Front End RAT   Subthreshold Leakage = " << fFRAT->rt_power.readOp.leakage  << " W" << endl;
			cerr << indent_str_next << "FP Front End RAT   Gate Leakage = " << fFRAT->rt_power.readOp.gate_leakage  << " W" << endl;
			cerr << indent_str_next << "Free List   Peak Dynamic = " << ifreeL->rt_power.readOp.dynamic*clockRate  << " W" << endl;
			cerr << indent_str_next << "Free List   Subthreshold Leakage = " << ifreeL->rt_power.readOp.leakage  << " W" << endl;
			cerr << indent_str_next << "Free List   Gate Leakage = " << fFRAT->rt_power.readOp.gate_leakage  << " W" << endl;
			if (coredynp.scheu_ty==PhysicalRegFile)
			{
				cerr << indent_str_next << "Int Retire RAT   Peak Dynamic = " << iRRAT->rt_power.readOp.dynamic*clockRate  << " W" << endl;
				cerr << indent_str_next << "Int Retire RAT   Subthreshold Leakage = " << iRRAT->rt_power.readOp.leakage  << " W" << endl;
				cerr << indent_str_next << "Int Retire RAT   Gate Leakage = " << iRRAT->rt_power.readOp.gate_leakage  << " W" << endl;
				cerr << indent_str_next << "FP Retire RAT   Peak Dynamic = " << fRRAT->rt_power.readOp.dynamic*clockRate  << " W" << endl;
				cerr << indent_str_next << "FP Retire RAT   Subthreshold Leakage = " << fRRAT->rt_power.readOp.leakage  << " W" << endl;
				cerr << indent_str_next << "FP Retire RAT   Gate Leakage = " << fRRAT->rt_power.readOp.gate_leakage  << " W" << endl;
				cerr << indent_str_next << "FP Free List   Peak Dynamic = " << ffreeL->rt_power.readOp.dynamic*clockRate  << " W" << endl;
				cerr << indent_str_next << "FP Free List   Subthreshold Leakage = " << ffreeL->rt_power.readOp.leakage  << " W" << endl;
				cerr << indent_str_next << "FP Free List   Gate Leakage = " << fFRAT->rt_power.readOp.gate_leakage  << " W" << endl;
			}
		}
		else
		{
			cerr << indent_str_next << "Int DCL   Peak Dynamic = " << idcl->rt_power.readOp.dynamic*clockRate  << " W" << endl;
			cerr << indent_str_next << "Int DCL   Subthreshold Leakage = " << idcl->rt_power.readOp.leakage  << " W" << endl;
			cerr << indent_str_next << "Int DCL   Gate Leakage = " << idcl->rt_power.readOp.gate_leakage  << " W" << endl;
			cerr << indent_str_next << "FP DCL   Peak Dynamic = " << fdcl->rt_power.readOp.dynamic*clockRate  << " W" << endl;
			cerr << indent_str_next << "FP DCL   Subthreshold Leakage = " << fdcl->rt_power.readOp.leakage  << " W" << endl;
			cerr << indent_str_next << "FP DCL   Gate Leakage = " << fdcl->rt_power.readOp.gate_leakage  << " W" << endl;
		}
	}
...
//	set_pppm(pppm_t, XML->sys.core[ithCore].issue_width,1, 1, 1);
//	cerr<<"Scheduler power="<<power.readOp.dynamic<<"leakage="<<power.readOp.leakage<<endl;
//	cerr<<"IW="<<int_inst_window->local_result.power.searchOp.dynamic * int_inst_window->stats_t.readAc.access +
...
//	cerr<<"selection"<<instruction_selection->power.readOp.dynamic<<"leakage"<<instruction_selection->power.readOp.leakage<<endl;
}
...
void SchedulerU::displayEnergy(uint32_t indent,int plevel,bool is_tdp)
{
	...
	if (is_tdp)
	{
		if (coredynp.core_ty==OOO)
		{
			cerr << indent_str << "Instruction Window:" << endl;
			cerr << indent_str_next << "Area = " << int_inst_window->area.get_area()*1e-6<< " mm^2" << endl;
			cerr << indent_str_next << "Peak Dynamic = " << int_inst_window->power.readOp.dynamic*clockRate << " W" << endl;
			cerr << indent_str_next << "Subthreshold Leakage = "
				<< (long_channel? int_inst_window->power.readOp.longer_channel_leakage:int_inst_window->power.readOp.leakage) <<" W" << endl;
			cerr << indent_str_next << "Gate Leakage = " << int_inst_window->power.readOp.gate_leakage << " W" << endl;
			cerr << indent_str_next << "Runtime Dynamic = " << int_inst_window->rt_power.readOp.dynamic/executionTime << " W" << endl;
			cerr <<endl;
			cerr << indent_str << "FP Instruction Window:" << endl;
			cerr << indent_str_next << "Area = " << fp_inst_window->area.get_area()*1e-6  << " mm^2" << endl;
			cerr << indent_str_next << "Peak Dynamic = " << fp_inst_window->power.readOp.dynamic*clockRate  << " W" << endl;
			cerr << indent_str_next << "Subthreshold Leakage = "
				<< (long_channel? fp_inst_window->power.readOp.longer_channel_leakage:fp_inst_window->power.readOp.leakage ) << " W" << endl;
			cerr << indent_str_next << "Gate Leakage = " << fp_inst_window->power.readOp.gate_leakage  << " W" << endl;
			cerr << indent_str_next << "Runtime Dynamic = " << fp_inst_window->rt_power.readOp.dynamic/executionTime << " W" << endl;
			cerr <<endl;
			if (XML->sys.core[ithCore].ROB_size >0)
			{
				cerr << indent_str<<"ROB:" << endl;
				cerr << indent_str_next << "Area = " << ROB->area.get_area() *1e-6 << " mm^2" << endl;
				cerr << indent_str_next << "Peak Dynamic = " << ROB->power.readOp.dynamic*clockRate  << " W" << endl;
				cerr << indent_str_next << "Subthreshold Leakage = "
				<< (long_channel? ROB->power.readOp.longer_channel_leakage:ROB->power.readOp.leakage)  << " W" << endl;
				cerr << indent_str_next << "Gate Leakage = " << ROB->power.readOp.gate_leakage  << " W" << endl;
				cerr << indent_str_next << "Runtime Dynamic = " << ROB->rt_power.readOp.dynamic/executionTime << " W" << endl;
				cerr <<endl;
			}
		}
		else if (coredynp.multithreaded)
	...
	else
	{
		if (coredynp.core_ty==OOO)
		{
			cerr << indent_str_next << "Instruction Window    Peak Dynamic = " << int_inst_window->rt_power.readOp.dynamic*clockRate << " W" << endl;
			cerr << indent_str_next << "Instruction Window    Subthreshold Leakage = " << int_inst_window->rt_power.readOp.leakage <<" W" << endl;
			cerr << indent_str_next << "Instruction Window    Gate Leakage = " << int_inst_window->rt_power.readOp.gate_leakage << " W" << endl;
			cerr << indent_str_next << "FP Instruction Window   Peak Dynamic = " << fp_inst_window->rt_power.readOp.dynamic*clockRate  << " W" << endl;
			cerr << indent_str_next << "FP Instruction Window   Subthreshold Leakage = " << fp_inst_window->rt_power.readOp.leakage  << " W" << endl;
			cerr << indent_str_next << "FP Instruction Window   Gate Leakage = " << fp_inst_window->rt_power.readOp.gate_leakage  << " W" << endl;
			if (XML->sys.core[ithCore].ROB_size >0)
			{
				cerr << indent_str_next << "ROB   Peak Dynamic = " << ROB->rt_power.readOp.dynamic*clockRate  << " W" << endl;
				cerr << indent_str_next << "ROB   Subthreshold Leakage = " << ROB->rt_power.readOp.leakage  << " W" << endl;
				cerr << indent_str_next << "ROB   Gate Leakage = " << ROB->rt_power.readOp.gate_leakage  << " W" << endl;
			}
		}
		else if (coredynp.multithreaded)
		{
...
void LoadStoreU::displayEnergy(uint32_t indent,int plevel,bool is_tdp)
{
	...
	if (is_tdp)
	{
		cerr << indent_str << "Data Cache:" << endl;
		cerr << indent_str_next << "Area = " << dcache.area.get_area()*1e-6<< " mm^2" << endl;
		cerr << indent_str_next << "Peak Dynamic = " << dcache.power.readOp.dynamic*clockRate << " W" << endl;
		cerr << indent_str_next << "Subthreshold Leakage = "
			<< (long_channel? dcache.power.readOp.longer_channel_leakage:dcache.power.readOp.leakage )<<" W" << endl;
		cerr << indent_str_next << "Gate Leakage = " << dcache.power.readOp.gate_leakage << " W" << endl;
		cerr << indent_str_next << "Runtime Dynamic = " << dcache.rt_power.readOp.dynamic/executionTime << " W" << endl;
		cerr <<endl;
		if (coredynp.core_ty==Inorder)
		{
			cerr << indent_str << "Load/Store Queue:" << endl;
			cerr << indent_str_next << "Area = " << LSQ->area.get_area()*1e-6  << " mm^2" << endl;
			cerr << indent_str_next << "Peak Dynamic = " << LSQ->power.readOp.dynamic*clockRate  << " W" << endl;
			cerr << indent_str_next << "Subthreshold Leakage = "
				<< (long_channel? LSQ->power.readOp.longer_channel_leakage:LSQ->power.readOp.leakage)  << " W" << endl;
			cerr << indent_str_next << "Gate Leakage = " << LSQ->power.readOp.gate_leakage  << " W" << endl;
			cerr << indent_str_next << "Runtime Dynamic = " << LSQ->rt_power.readOp.dynamic/executionTime << " W" << endl;
			cerr <<endl;
		}
		else

		{
			cerr << indent_str << "LoadQ:" << endl;
			cerr << indent_str_next << "Area = " << LoadQ->area.get_area() *1e-6 << " mm^2" << endl;
			cerr << indent_str_next << "Peak Dynamic = " << LoadQ->power.readOp.dynamic*clockRate  << " W" << endl;
			cerr << indent_str_next << "Subthreshold Leakage = "
				<< (long_channel? LoadQ->power.readOp.longer_channel_leakage:LoadQ->power.readOp.leakage)  << " W" << endl;
			cerr << indent_str_next << "Gate Leakage = " << LoadQ->power.readOp.gate_leakage  << " W" << endl;
			cerr << indent_str_next << "Runtime Dynamic = " << LoadQ->rt_power.readOp.dynamic/executionTime << " W" << endl;
			cerr <<endl;
			cerr << indent_str<< "StoreQ:" << endl;
			cerr << indent_str_next << "Area = " << LSQ->area.get_area()  *1e-6<< " mm^2" << endl;
			cerr << indent_str_next << "Peak Dynamic = " << LSQ->power.readOp.dynamic*clockRate  << " W" << endl;
			cerr << indent_str_next << "Subthreshold Leakage = "
				<< (long_channel? LSQ->power.readOp.longer_channel_leakage:LSQ->power.readOp.leakage)  << " W" << endl;
	...
	{
		cerr << indent_str_next << "Data Cache    Peak Dynamic = " << dcache.rt_power.readOp.dynamic*clockRate << " W" << endl;
		cerr << indent_str_next << "Data Cache    Subthreshold Leakage = " << dcache.rt_power.readOp.leakage <<" W" << endl;
		cerr << indent_str_next << "Data Cache    Gate Leakage = " << dcache.rt_power.readOp.gate_leakage << " W" << endl;
		if (coredynp.core_ty==Inorder)
		{
			cerr << indent_str_next << "Load/Store Queue   Peak Dynamic = " << LSQ->rt_power.readOp.dynamic*clockRate  << " W" << endl;
			cerr << indent_str_next << "Load/Store Queue   Subthreshold Leakage = " << LSQ->rt_power.readOp.leakage  << " W" << endl;
			cerr << indent_str_next << "Load/Store Queue   Gate Leakage = " << LSQ->rt_power.readOp.gate_leakage  << " W" << endl;
		}
		else
		{
			cerr << indent_str_next << "LoadQ   Peak Dynamic = " << LoadQ->rt_power.readOp.dynamic*clockRate  << " W" << endl;
			cerr << indent_str_next << "LoadQ   Subthreshold Leakage = " << LoadQ->rt_power.readOp.leakage  << " W" << endl;
			cerr << indent_str_next << "LoadQ   Gate Leakage = " << LoadQ->rt_power.readOp.gate_leakage  << " W" << endl;
			cerr << indent_str_next << "StoreQ   Peak Dynamic = " << LSQ->rt_power.readOp.dynamic*clockRate  << " W" << endl;
			cerr << indent_str_next << "StoreQ   Subthreshold Leakage = " << LSQ->rt_power.readOp.leakage  << " W" << endl;
			cerr << indent_str_next << "StoreQ   Gate Leakage = " << LSQ->rt_power.readOp.gate_leakage  << " W" << endl;
		}
	}
...
void MemManU::displayEnergy(uint32_t indent,int plevel,bool is_tdp)
{
	...
	if (is_tdp)
	{
		cerr << indent_str << "Itlb:" << endl;
		cerr << indent_str_next << "Area = " << itlb->area.get_area()*1e-6<< " mm^2" << endl;
		cerr << indent_str_next << "Peak Dynamic = " << itlb->power.readOp.dynamic*clockRate << " W" << endl;
		cerr << indent_str_next << "Subthreshold Leakage = "
			<< (long_channel? itlb->power.readOp.longer_channel_leakage:itlb->power.readOp.leakage) <<" W" << endl;
		cerr << indent_str_next << "Gate Leakage = " << itlb->power.readOp.gate_leakage << " W" << endl;
		cerr << indent_str_next << "Runtime Dynamic = " << itlb->rt_power.readOp.dynamic/executionTime << " W" << endl;
		cerr <<endl;
		cerr << indent_str<< "Dtlb:" << endl;
		cerr << indent_str_next << "Area = " << dtlb->area.get_area()*1e-6  << " mm^2" << endl;
		cerr << indent_str_next << "Peak Dynamic = " << dtlb->power.readOp.dynamic*clockRate  << " W" << endl;
		cerr << indent_str_next << "Subthreshold Leakage = "
			<< (long_channel? dtlb->power.readOp.longer_channel_leakage:dtlb->power.readOp.leakage)  << " W" << endl;
		cerr << indent_str_next << "Gate Leakage = " << dtlb->power.readOp.gate_leakage  << " W" << endl;
		cerr << indent_str_next << "Runtime Dynamic = " << dtlb->rt_power.readOp.dynamic/executionTime << " W" << endl;
		cerr <<endl;
	}
	else
	{
		cerr << indent_str_next << "Itlb    Peak Dynamic = " << itlb->rt_power.readOp.dynamic*clockRate << " W" << endl;
		cerr << indent_str_next << "Itlb    Subthreshold Leakage = " << itlb->rt_power.readOp.leakage <<" W" << endl;
		cerr << indent_str_next << "Itlb    Gate Leakage = " << itlb->rt_power.readOp.gate_leakage << " W" << endl;
		cerr << indent_str_next << "Dtlb   Peak Dynamic = " << dtlb->rt_power.readOp.dynamic*clockRate  << " W" << endl;
		cerr << indent_str_next << "Dtlb   Subthreshold Leakage = " << dtlb->rt_power.readOp.leakage  << " W" << endl;
		cerr << indent_str_next << "Dtlb   Gate Leakage = " << dtlb->rt_power.readOp.gate_leakage  << " W" << endl;
	}
...
void RegFU::displayEnergy(uint32_t indent,int plevel,bool is_tdp)
{
	...
	if (is_tdp)
	{	cerr << indent_str << "Integer RF:" << endl;
		cerr << indent_str_next << "Area = " << IRF->area.get_area()*1e-6<< " mm^2" << endl;
		cerr << indent_str_next << "Peak Dynamic = " << IRF->power.readOp.dynamic*clockRate << " W" << endl;
		cerr << indent_str_next << "Subthreshold Leakage = "
			<< (long_channel? IRF->power.readOp.longer_channel_leakage:IRF->power.readOp.leakage) <<" W" << endl;
		cerr << indent_str_next << "Gate Leakage = " << IRF->power.readOp.gate_leakage << " W" << endl;
		cerr << indent_str_next << "Runtime Dynamic = " << IRF->rt_power.readOp.dynamic/executionTime << " W" << endl;
		cerr <<endl;
		cerr << indent_str<< "Floating Point RF:" << endl;
		cerr << indent_str_next << "Area = " << FRF->area.get_area()*1e-6  << " mm^2" << endl;
		cerr << indent_str_next << "Peak Dynamic = " << FRF->power.readOp.dynamic*clockRate  << " W" << endl;
		cerr << indent_str_next << "Subthreshold Leakage = "
			<< (long_channel? FRF->power.readOp.longer_channel_leakage:FRF->power.readOp.leakage)  << " W" << endl;
		cerr << indent_str_next << "Gate Leakage = " << FRF->power.readOp.gate_leakage  << " W" << endl;
		cerr << indent_str_next << "Runtime Dynamic = " << FRF->rt_power.readOp.dynamic/executionTime << " W" << endl;
		cerr <<endl;
		if (coredynp.regWindowing)
		{
			cerr << indent_str << "Register Windows:" << endl;
			cerr << indent_str_next << "Area = " << RFWIN->area.get_area() *1e-6 << " mm^2" << endl;
			cerr << indent_str_next << "Peak Dynamic = " << RFWIN->power.readOp.dynamic*clockRate  << " W" << endl;
			cerr << indent_str_next << "Subthreshold Leakage = "
				<< (long_channel? RFWIN->power.readOp.longer_channel_leakage:RFWIN->power.readOp.leakage)  << " W" << endl;
			cerr << indent_str_next << "Gate Leakage = " << RFWIN->power.readOp.gate_leakage  << " W" << endl;
			cerr << indent_str_next << "Runtime Dynamic = " << RFWIN->rt_power.readOp.dynamic/executionTime << " W" << endl;
			cerr <<endl;
		}
	}
	else
	{
		cerr << indent_str_next << "Integer RF    Peak Dynamic = " << IRF->rt_power.readOp.dynamic*clockRate << " W" << endl;
		cerr << indent_str_next << "Integer RF    Subthreshold Leakage = " << IRF->rt_power.readOp.leakage <<" W" << endl;
		cerr << indent_str_next << "Integer RF    Gate Leakage = " << IRF->rt_power.readOp.gate_leakage << " W" << endl;
		cerr << indent_str_next << "Floating Point RF   Peak Dynamic = " << FRF->rt_power.readOp.dynamic*clockRate  << " W" << endl;
		cerr << indent_str_next << "Floating Point RF   Subthreshold Leakage = " << FRF->rt_power.readOp.leakage  << " W" << endl;
		cerr << indent_str_next << "Floating Point RF   Gate Leakage = " << FRF->rt_power.readOp.gate_leakage  << " W" << endl;
		if (coredynp.regWindowing)
		{
			cerr << indent_str_next << "Register Windows   Peak Dynamic = " << RFWIN->rt_power.readOp.dynamic*clockRate  << " W" << endl;
			cerr << indent_str_next << "Register Windows   Subthreshold Leakage = " << RFWIN->rt_power.readOp.leakage  << " W" << endl;
			cerr << indent_str_next << "Register Windows   Gate Leakage = " << RFWIN->rt_power.readOp.gate_leakage  << " W" << endl;
		}
	}
}
...
//	cerr << indent_str_next << "Results Broadcast Bus Area = " << bypass->area.get_area() *1e-6 << " mm^2" << endl;
	if (is_tdp)
	{
		cerr << indent_str << "Register Files:" << endl;
		cerr << indent_str_next << "Area = " << rfu->area.get_area()*1e-6<< " mm^2" << endl;
		cerr << indent_str_next << "Peak Dynamic = " << rfu->power.readOp.dynamic*clockRate << " W" << endl;
		cerr << indent_str_next << "Subthreshold Leakage = "
			<< (long_channel? rfu->power.readOp.longer_channel_leakage:rfu->power.readOp.leakage) <<" W" << endl;
		cerr << indent_str_next << "Gate Leakage = " << rfu->power.readOp.gate_leakage << " W" << endl;
		cerr << indent_str_next << "Runtime Dynamic = " << rfu->rt_power.readOp.dynamic/executionTime << " W" << endl;
		cerr <<endl;
		if (plevel>3){
			rfu->displayEnergy(indent+4,is_tdp);
		}
		cerr << indent_str << "Instruction Scheduler:" << endl;
		cerr << indent_str_next << "Area = " << scheu->area.get_area()*1e-6  << " mm^2" << endl;
		cerr << indent_str_next << "Peak Dynamic = " << scheu->power.readOp.dynamic*clockRate  << " W" << endl;
		cerr << indent_str_next << "Subthreshold Leakage = "
			<< (long_channel? scheu->power.readOp.longer_channel_leakage:scheu->power.readOp.leakage)  << " W" << endl;
		cerr << indent_str_next << "Gate Leakage = " << scheu->power.readOp.gate_leakage  << " W" << endl;
		cerr << indent_str_next << "Runtime Dynamic = " << scheu->rt_power.readOp.dynamic/executionTime << " W" << endl;
		cerr <<endl;
		if (plevel>3){
		...
		cerr << indent_str << "Results Broadcast Bus:" << endl;
		cerr << indent_str_next << "Area Overhead = " << bypass.area.get_area()*1e-6  << " mm^2" << endl;
		cerr << indent_str_next << "Peak Dynamic = " << bypass.power.readOp.dynamic*clockRate  << " W" << endl;
		cerr << indent_str_next << "Subthreshold Leakage = "
			<< (long_channel? bypass.power.readOp.longer_channel_leakage:bypass.power.readOp.leakage ) << " W" << endl;
		cerr << indent_str_next << "Gate Leakage = " << bypass.power.readOp.gate_leakage  << " W" << endl;
		cerr << indent_str_next << "Runtime Dynamic = " << bypass.rt_power.readOp.dynamic/executionTime << " W" << endl;
		cerr <<endl;
	}
	else
	{
		cerr << indent_str_next << "Register Files    Peak Dynamic = " << rfu->rt_power.readOp.dynamic*clockRate << " W" << endl;
		cerr << indent_str_next << "Register Files    Subthreshold Leakage = " << rfu->rt_power.readOp.leakage <<" W" << endl;
		cerr << indent_str_next << "Register Files    Gate Leakage = " << rfu->rt_power.readOp.gate_leakage << " W" << endl;
		cerr << indent_str_next << "Instruction Sheduler   Peak Dynamic = " << scheu->rt_power.readOp.dynamic*clockRate  << " W" << endl;
		cerr << indent_str_next << "Instruction Sheduler   Subthreshold Leakage = " << scheu->rt_power.readOp.leakage  << " W" << endl;
		cerr << indent_str_next << "Instruction Sheduler   Gate Leakage = " << scheu->rt_power.readOp.gate_leakage  << " W" << endl;
		cerr << indent_str_next << "Results Broadcast Bus   Peak Dynamic = " << bypass.rt_power.readOp.dynamic*clockRate  << " W" << endl;
		cerr << indent_str_next << "Results Broadcast Bus   Subthreshold Leakage = " << bypass.rt_power.readOp.leakage  << " W" << endl;
		cerr << indent_str_next << "Results Broadcast Bus   Gate Leakage = " << bypass.rt_power.readOp.gate_leakage  << " W" << endl;
	}
...
//			cerr << "IFU = " << ifu->power.readOp.dynamic*clockRate  << " W" << endl;
			ifu->power = ifu->power + corepipe->power*pppm_t;
//			cerr << "IFU = " << ifu->power.readOp.dynamic*clockRate  << " W" << endl;
//			cerr << "1/4 pipe = " << corepipe->power.readOp.dynamic*clockRate/num_units  << " W" << endl;
			power     = power + ifu->power;
//			cerr << "core = " << power.readOp.dynamic*clockRate  << " W" << endl;
		}
		if (lsu->exist)
		{
...
//			cerr << "LSU = " << lsu->power.readOp.dynamic*clockRate  << " W" << endl;
...
//			cerr << "EXE = " << exu->power.readOp.dynamic*clockRate  << " W" << endl;
...
//			cerr << "MMU = " << mmu->power.readOp.dynamic*clockRate  << " W" << endl;
...
void Core::displayEnergy(uint32_t indent,int plevel,bool is_tdp)
{
	...
	if (is_tdp)
	{
		...
		cerr << indent_str << "Peak Dynamic = " << power.readOp.dynamic*clockRate << " W" << endl;
		...
		cerr << indent_str << "Runtime Dynamic = " << rt_power.readOp.dynamic/executionTime << " W" << endl;
		cerr<<endl;
		if (ifu->exist)
		{
			cerr << indent_str << "Instruction Fetch Unit:" << endl;
			cerr << indent_str_next << "Area = " << ifu->area.get_area()*1e-6<< " mm^2" << endl;
			cerr << indent_str_next << "Peak Dynamic = " << ifu->power.readOp.dynamic*clockRate << " W" << endl;
			cerr << indent_str_next << "Subthreshold Leakage = "
				<< (long_channel? ifu->power.readOp.longer_channel_leakage:ifu->power.readOp.leakage) <<" W" << endl;
			//cerr << indent_str_next << "Subthreshold Leakage = " << ifu->power.readOp.longer_channel_leakage <<" W" << endl;
			cerr << indent_str_next << "Gate Leakage = " << ifu->power.readOp.gate_leakage << " W" << endl;
			cerr << indent_str_next << "Runtime Dynamic = " << ifu->rt_power.readOp.dynamic/executionTime << " W" << endl;
			cerr <<endl;
			if (plevel >2){
		...
		if (coredynp.core_ty==OOO)
		{
			if (rnu->exist)
			{
				cerr << indent_str<< "Renaming Unit:" << endl;
				cerr << indent_str_next << "Area = " << rnu->area.get_area()*1e-6  << " mm^2" << endl;
				cerr << indent_str_next << "Peak Dynamic = " << rnu->power.readOp.dynamic*clockRate  << " W" << endl;
				cerr << indent_str_next << "Subthreshold Leakage = "
					<< (long_channel? rnu->power.readOp.longer_channel_leakage:rnu->power.readOp.leakage)  << " W" << endl;
				//cerr << indent_str_next << "Subthreshold Leakage = " << rnu->power.readOp.longer_channel_leakage  << " W" << endl;
				cerr << indent_str_next << "Gate Leakage = " << rnu->power.readOp.gate_leakage  << " W" << endl;
				cerr << indent_str_next << "Runtime Dynamic = " << rnu->rt_power.readOp.dynamic/executionTime << " W" << endl;
				cerr <<endl;
				if (plevel >2){
		...
		if (lsu->exist)
		{
			cerr << indent_str<< "Load Store Unit:" << endl;
			cerr << indent_str_next << "Area = " << lsu->area.get_area()*1e-6  << " mm^2" << endl;
			cerr << indent_str_next << "Peak Dynamic = " << lsu->power.readOp.dynamic*clockRate  << " W" << endl;
			cerr << indent_str_next << "Subthreshold Leakage = "
				<< (long_channel? lsu->power.readOp.longer_channel_leakage:lsu->power.readOp.leakage ) << " W" << endl;
			//cerr << indent_str_next << "Subthreshold Leakage = " << lsu->power.readOp.longer_channel_leakage  << " W" << endl;
			cerr << indent_str_next << "Gate Leakage = " << lsu->power.readOp.gate_leakage  << " W" << endl;
			cerr << indent_str_next << "Runtime Dynamic = " << lsu->rt_power.readOp.dynamic/executionTime << " W" << endl;
			cerr <<endl;
			if (plevel >2){
		...
		if (mmu->exist)
		{
			cerr << indent_str<< "Memory Management Unit:" << endl;
			cerr << indent_str_next << "Area = " << mmu->area.get_area() *1e-6 << " mm^2" << endl;
			cerr << indent_str_next << "Peak Dynamic = " << mmu->power.readOp.dynamic*clockRate  << " W" << endl;
			cerr << indent_str_next << "Subthreshold Leakage = "
				<< (long_channel? mmu->power.readOp.longer_channel_leakage:mmu->power.readOp.leakage)   << " W" << endl;
			//cerr << indent_str_next << "Subthreshold Leakage = " << mmu->power.readOp.longer_channel_leakage   << " W" << endl;
			cerr << indent_str_next << "Gate Leakage = " << mmu->power.readOp.gate_leakage  << " W" << endl;
			cerr << indent_str_next << "Runtime Dynamic = " << mmu->rt_power.readOp.dynamic/executionTime << " W" << endl;
			cerr <<endl;
			if (plevel >2){
		...
		if (exu->exist)
		{
			cerr << indent_str<< "Execution Unit:" << endl;
			cerr << indent_str_next << "Area = " << exu->area.get_area()  *1e-6<< " mm^2" << endl;
			cerr << indent_str_next << "Peak Dynamic = " << exu->power.readOp.dynamic*clockRate  << " W" << endl;
			cerr << indent_str_next << "Subthreshold Leakage = "
				<< (long_channel? exu->power.readOp.longer_channel_leakage:exu->power.readOp.leakage)   << " W" << endl;
			//cerr << indent_str_next << "Subthreshold Leakage = " << exu->power.readOp.longer_channel_leakage << " W" << endl;
			cerr << indent_str_next << "Gate Leakage = " << exu->power.readOp.gate_leakage  << " W" << endl;
			cerr << indent_str_next << "Runtime Dynamic = " << exu->rt_power.readOp.dynamic/executionTime << " W" << endl;
			cerr <<endl;
			if (plevel >2){
...
//			if (undiffCore->exist)
//			{
//				cerr << indent_str << "Undifferentiated Core" << endl;
//				cerr << indent_str_next << "Area = " << undiffCore->area.get_area()*1e-6<< " mm^2" << endl;
//				cerr << indent_str_next << "Peak Dynamic = " << undiffCore->power.readOp.dynamic*clockRate << " W" << endl;
////				cerr << indent_str_next << "Subthreshold Leakage = " << undiffCore->power.readOp.leakage <<" W" << endl;
//				cerr << indent_str_next << "Subthreshold Leakage = "
//								<< (long_channel? undiffCore->power.readOp.longer_channel_leakage:undiffCore->power.readOp.leakage)   << " W" << endl;
//				cerr << indent_str_next << "Gate Leakage = " << undiffCore->power.readOp.gate_leakage << " W" << endl;
//				//		cerr << indent_str_next << "Runtime Dynamic = " << undiffCore->rt_power.readOp.dynamic/executionTime << " W" << endl;
//				cerr <<endl;
//			}
//		}
		if (XML->sys.Private_L2)
...
//	}
	...
	{
		cerr<<"Invalid Core Type"<<endl;
		exit(0);
	}
//	if (coredynp.core_ty==OOO)
//	{
//		cerr<<"OOO processor models are being updated and will be available in next release"<<endl;
//		exit(0);
//	}
	if (!((coredynp.scheu_ty==PhysicalRegFile)||(coredynp.scheu_ty==ReservationStation)))
	{
		cerr<<"Invalid OOO Scheduler Type"<<endl;
		exit(0);
	}

	if (!((coredynp.rm_ty ==RAMbased)||(coredynp.rm_ty ==CAMbased)))
	{
		cerr<<"Invalid OOO Renaming Type"<<endl;
		exit(0);
	}
