...
void selection_logic::leakage_feedback(double temperature)
{
 ...
 fu_type(fu_type_)
{
	...
	{
		cerr<<"Unknown Functional Unit Type"<<endl;
		exit(0);
	}
...
//	cerr << indent_str_next << "Results Broadcast Bus Area = " << bypass->area.get_area() *1e-6 << " mm^2" << endl;
	if (is_tdp)
	{
		if (fu_type == FPU)
		{
			cerr << indent_str << "Floating Point Units (FPUs) (Count: "<< coredynp.num_fpus <<" ):" << endl;
			cerr << indent_str_next << "Area = " << area.get_area()*1e-6  << " mm^2" << endl;
			cerr << indent_str_next << "Peak Dynamic = " << power.readOp.dynamic*clockRate  << " W" << endl;
//			cerr << indent_str_next << "Subthreshold Leakage = " << power.readOp.leakage  << " W" << endl;
			cerr << indent_str_next<< "Subthreshold Leakage = "
						<< (long_channel? power.readOp.longer_channel_leakage:power.readOp.leakage) <<" W" << endl;
			cerr << indent_str_next << "Gate Leakage = " << power.readOp.gate_leakage  << " W" << endl;
			cerr << indent_str_next << "Runtime Dynamic = " << rt_power.readOp.dynamic/executionTime << " W" << endl;
			cerr <<endl;
		}
		else if (fu_type == ALU)
		{
			cerr << indent_str << "Integer ALUs (Count: "<< coredynp.num_alus <<" ):" << endl;
			cerr << indent_str_next << "Area = " << area.get_area()*1e-6  << " mm^2" << endl;
			cerr << indent_str_next << "Peak Dynamic = " << power.readOp.dynamic*clockRate  << " W" << endl;
//			cerr << indent_str_next << "Subthreshold Leakage = " << power.readOp.leakage  << " W" << endl;
			cerr << indent_str_next<< "Subthreshold Leakage = "
		...
		{
			cerr << indent_str << "Complex ALUs (Mul/Div) (Count: "<< coredynp.num_muls <<" ):" << endl;
			cerr << indent_str_next << "Area = " << area.get_area()*1e-6  << " mm^2" << endl;
			cerr << indent_str_next << "Peak Dynamic = " << power.readOp.dynamic*clockRate  << " W" << endl;
//			cerr << indent_str_next << "Subthreshold Leakage = " << power.readOp.leakage  << " W" << endl;
			cerr << indent_str_next<< "Subthreshold Leakage = "
...
//		cerr<<power.readOp.dynamic << "dynamic" <<endl;
//		cerr<<power.readOp.sc << "sc" << endl;
...
void UndiffCore::displayEnergy(uint32_t indent,int plevel,bool is_tdp)
{
	...
	if (is_tdp)
	{
		cerr << indent_str << "UndiffCore:" << endl;
		cerr << indent_str_next << "Area = " << area.get_area()*1e-6<< " mm^2" << endl;
		cerr << indent_str_next << "Peak Dynamic = " << power.readOp.dynamic*clockRate << " W" << endl;
		//cerr << indent_str_next << "Subthreshold Leakage = " << power.readOp.leakage <<" W" << endl;
		cerr << indent_str_next<< "Subthreshold Leakage = "
...
/* This is runtime leakage feedback control. It updates the leakage power for the given temperature.
   This leakage feedback algorithm is based on the modified creator function of inst_decoder, where
   predecoder and final decoder are separately accessible.
   Modified by William Song (wjhsong@gatech.edu), Georgia Tech and Sandia National Laboratories. */
void inst_decoder::leakage_feedback(double temperature)
{
  ...
  set_pppm(pppm_t, squencer_passes*num_decoder_segments, num_decoder_segments, squencer_passes*num_decoder_segments, num_decoder_segments);
  dec_power = pre_dec->power*pppm_t;

  // copy back only leakage updates
  pre_dec->power.readOp.leakage = dec_power.readOp.leakage;
  pre_dec->power.readOp.gate_leakage = dec_power.readOp.gate_leakage;
  ...
  dec_power = final_dec->power*pppm_t;

  // copy back only leakage updates
  final_dec->power.readOp.leakage = dec_power.readOp.leakage;
  final_dec->power.readOp.gate_leakage = dec_power.readOp.gate_leakage;
