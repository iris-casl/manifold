...
void Router::leakage_feedback(double temperature)
{
  // Router does not maintain buffer structure. It needs to be restored in order for leakage update.
  DynamicParameter dyn_p;
  dyn_p.is_tag      = false;
  dyn_p.pure_cam    = false;
  dyn_p.fully_assoc = false;
  ...
  buffer.power.readOp.leakage = buff.power.readOp.leakage;
  buffer.power.readOp.gate_leakage = buff.power.readOp.gate_leakage;


  // Restoring the crossbar and updating leakage
  Crossbar c_b(I, O, flit_size);
  c_b.compute_power();
  ...
  // Restoring the arbiter and updating leakage
  Arbiter vcarb(vc_count, flit_size, buffer.area.w);
  Arbiter cbarb(I, flit_size, crossbar.area.w);
  vcarb.compute_power();
  cbarb.compute_power();
  ...
  // Final update of the router leakage
  double pppm_t[4]    = {1,I,I,1};
  power = (buffer.power*pppm_t + crossbar.power + arbiter.power)*pppm_lkg;
}
