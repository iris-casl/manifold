...
void SharedCache::computeEnergy(bool is_tdp)
{
	if (is_tdp)
	{
		...
		{
			//init stats for Peak
			unicache.caches->stats_t.readAc.access  = .67*unicache.caches->l_ip.num_rw_ports*cachep.duty_cycle;
			...
			unicache.caches->stats_t.writeAc.access = .33*unicache.caches->l_ip.num_rw_ports*cachep.duty_cycle;
...
//		cerr<<"unicache.caches->local_result.power.readOp.dynamic"<<unicache.caches->local_result.power.readOp.dynamic<<endl;
//		cerr<<"unicache.caches->local_result.power.writeOp.dynamic"<<unicache.caches->local_result.power.writeOp.dynamic<<endl;
	}
	else
	{
...
void SharedCache::displayEnergy(uint32_t indent,bool is_tdp)
{
	...
	if (is_tdp)
	{
		cerr << (XML->sys.Private_L2? indent_str:"")<< cachep.name << endl;
		cerr << indent_str << "Area = " << area.get_area()*1e-6<< " mm^2" << endl;
		cerr << indent_str << "Peak Dynamic = " << power.readOp.dynamic*cachep.clockRate << " W" << endl;
		...
		cerr << indent_str << "Runtime Dynamic = " << rt_power.readOp.dynamic/cachep.executionTime << " W" << endl;
		cerr <<endl;
	}
	else
	{
...
//  ///cerr<<"llCache.maxPower=" <<llCache.maxPower<<endl;
//
...
//		                        +directory.caches.l_ip.num_rd_ports*directory.caches.local_result.power.readOp.dynamic+directory.caches.l_ip.num_wr_ports*directory.caches.local_result.power.writeOp.dynamic
//		                        +directory.caches.l_ip.num_se_rd_ports*directory.caches.local_result.power.readOp.dynamic)*clockRate;
//		  ///cerr<<"directory.maxPower=" <<directory.maxPower<<endl;
//
...
//		  cerr<<"CC area="<<cc.area.get_area()*1e-6<<endl;
//		  cerr<<"CC Power="<<cc.power.readOp.dynamic<<endl;
//		  ccTot.area.set_area(cc.area.get_area()*1e-6);
//		  ccTot.power = cc.power;
//		  cerr<<"DC energy per access" << cc.power.readOp.dynamic/clockRate/8;
//	  }
...
//	  		  ///cerr<<"directory1.maxPower=" <<directory1.maxPower<<endl;
//
...
//
//	  		  cerr<<"CC area="<<(cc.area.get_area()+cc1.area.get_area())*1e-6<<endl;
//			  cerr<<"CC Power="<<cc.power.readOp.dynamic + cc1.power.readOp.dynamic <<endl;
...
//  }
//
//cerr<<"L2cache size="<<L2Tot.area.get_area()*1e-6<<endl;
//cerr<<"L2cache dynamic power="<<L2Tot.power.readOp.dynamic<<endl;
//cerr<<"L2cache laeakge power="<<L2Tot.power.readOp.leakage<<endl;
//
//  ///cerr<<"llCache.maxPower=" <<llCache.maxPower<<endl;
//
//
//  maxPower          +=  llCache.maxPower;
//  ///cerr<<"maxpower=" <<maxPower<<endl;
//
////  maxPower	  +=  pipeLogicCache.power.readOp.dynamic*clockRate;
////  ///cerr<<"pipeLogic.power="<<pipeLogicCache.power.readOp.dynamic*clockRate<<endl;
////  ///cerr<<"maxpower=" <<maxPower<<endl;
////
////  maxPower	  +=  pipeLogicDirectory.power.readOp.dynamic*clockRate;
////  ///cerr<<"pipeLogic.power="<<pipeLogicDirectory.power.readOp.dynamic*clockRate<<endl;
////  ///cerr<<"maxpower=" <<maxPower<<endl;
////
////  //clock power
////  maxPower += clockNetwork.total_power.readOp.dynamic*clockRate;
////  ///cerr<<"clockNetwork.total_power="<<clockNetwork.total_power.readOp.dynamic*clockRate<<endl;
////  ///cerr<<"maxpower=" <<maxPower<<endl;
//
//}
