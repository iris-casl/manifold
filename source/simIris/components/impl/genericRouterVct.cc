...
//                    if(!send_early_credit)
                ...
                {
                    if(in_buffers[iport].get_occupancy(0)==0)
                    {
#ifdef _DEBUG_ROUTER
                        _DBG(" Waiting for flits on inch %d. Credits are %d",
                             iport,downstream_credits[oport][0]);
#endif
                    }
                    ...
                    {
#ifdef _DEBUG_ROUTER
                        _DBG(" Waiting for downstream credits on oport %d 0 %d ", oport, 0);
#endif
                       ticking = true;
                    }
                }
    ...
    for( uint i=0; i<(ports*vcs); i++)
    {
        if( input_buffer_state[i].pipe_stage == IB )
        {
                ...
                && downstream_credits[oport][0] == credits)
            {
                ...
                ticking = true;
#ifdef _DEBUG_ROUTER
                _DBG(" Req SWA for inport %d oport %d  ", iport, oport);
#endif
            }
#ifdef _DEBUG_ROUTER
            else
            {
                if (swa.is_requested(oport, iport))
                {
                    _DBG("Cant req SWA. SWA full inport:%d oport:%d ",iport,oport);
                }
